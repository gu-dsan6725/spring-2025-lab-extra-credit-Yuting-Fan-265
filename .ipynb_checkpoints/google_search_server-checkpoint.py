...
@mcp.tool()
def google_search(query: str, num: int = 5, sleep_interval: int = 5) -> str:
    """
    Searches Google for a query and retrieves the text content of the top results.

    Args:
        query (str): The search query.
        num (int): Number of search results to retrieve.
        sleep_interval (int): Pause time between requests to avoid getting blocked.

    Returns:
        str: Combined text content from retrieved web pages.
    """

    results = list(search(query, num_results=num, sleep_interval=sleep_interval))
    ...
    # Combine extracted text for context
    return "\n".join(context_texts)
